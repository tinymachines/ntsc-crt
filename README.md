...
| `ntsc-wasm` | The browser bridge: dot frames to RGBA, the drift policy with counted duplicates and drops, plain-Rust core so the native bench measures the page's own code. Since 0.2.5 it also hands the encoded samples and the decoder's constants out, for a decoder elsewhere (the console page's WebGPU decode), held to push_frame's bytes.|
...
```bash
cargo test --workspace              # 61 tests: residues, data consistency,
