...
and M2 (RGB encode, the comb rungs, WASM, the measured budget) are built
and closed; the milestone logs are `docs/m0-report.md`,
`docs/m1-report.md` and `docs/m2-report.md`. Seven crates:
...
```bash
cargo test --workspace              # 45 tests: residues, data consistency,
...
MUTATE=1 cargo test --workspace --no-fail-fast   # must go red: 27 tests
